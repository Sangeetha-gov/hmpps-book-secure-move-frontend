...
function getLocationBreakdown(moves, direction = 'from_location') {
  ...
  let output = 'Rank,Location,Number of moves\n'
  forEach(sorted, (location, i) => {
    output += `${i + 1},${location.title},${location.count}\n`
  })
...
function getLeadTimeBreakdown(moves) {
  ...
  const filtered = moves.map(move => {
    ...
    return {
      pickupType: move.from_location?.location_type,
      leadTime: differenceInDays(leftDate, rightDate),
    }
  })
  ...
  output += '\n'
  forEach(dataBreakdown, (breakdown, time) => {
    output += time === '0' ? 'Less than 1 day' : `${time} days`
    forEach(breakdown, (count, origin) => {
      output += `,${count}`
    })
    output += '\n'
...
router.get('/refresh', async (req, res, next) => {
  try {
    const moves = await moveService.getActive({
      createdateRange: getCurrentMonthAsRange(),
    })
    ...
    await updateChart(typeBreakdown, 'YMkvs')
    await updateChart(pickups, 'k3R82')
    await updateChart(dropoffs, 'UMF0k')
    await updateChart(bookingTimes, 'A1NVp')
    await updateChart(leadTimes, 'eHJzI')
    await updateChart(cancellationReasons, '6DxZD')
    await updateChart(percentageOfCancellations, 'ckRbc')
